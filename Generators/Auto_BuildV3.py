    ...
    for i in range(1, num_segments):
        ...
        for a in (4, 5, 6):
            for b in (1, 2, 3):
                ...
                tendon_name = f"T_S{i}S{i+1}_{idx}_R{robot_number}"
                tendon_xml += (
                    f'    <spatial name="{tendon_name}" '
                    'width="0.0004" frictionloss="0.1" '
    ...
    for i in range(1, num_segments):
        ...
        for a in (4, 5, 6):
            for b in (1, 2, 3):
                low = 0.003 if (a - 3) == b else 0.009
                tendon_name = f"T_S{i}S{i+1}_{idx}_R{robot_number}"
                muscle_name = f"A_S{i}S{i+1}_{idx}_R{robot_number}"
                actuator_xml += (
                    f'    <muscle name="{muscle_name}" '
                    'ctrllimited="true" '
                    f'lengthrange="{low:.3f} 0.1" '
                    'ctrlrange="0 1" force="50" '
                    f'tendon="{tendon_name}"/>\n'
                )
                idx += 1
    actuator_xml += '</actuator>\n\n'

    # -----------------------------------------------------
    # 7) RETURN FULL XML STRING
    # -----------------------------------------------------
    return asset_xml + body_xml + tendon_xml + actuator_xml
